...
public final class Parser {
    ...
    public Ast.Source parseSource() throws ParseException {
        ...
        if(peek("LIST")||peek("VAR")||peek("VAL")){
            if(tokens.has(0)){
                throw new ParseException("Global After Functions",
                        tokens.get(0).getIndex());
            }
            else throw new ParseException("Global After Functions", tokens.get(-
    ...
    public Ast.Global parseGlobal() throws ParseException {
        if (peek("LIST")){
            ...
            if(!peek(";")){
                if(tokens.has(0)){
                    throw new ParseException("Expected ; After Global",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Expected ; After Global",
        ...
        throw new ParseException("Unknown Global Error",
                tokens.get(0).getIndex());
    }
    /**
        ...
        if(peek("LIST")) {
            ...
            if (peek(Token.Type.IDENTIFIER)) {
                if(peek("LIST")||peek("VAR")||peek("VAL")||peek("FUN")||
                        peek("LET")||peek("SWITCH")||peek("CASE")||peek("DEFAULT")||peek("END")||
                        peek("IF")||peek("DO")||peek("ELSE")||peek("WHILE")||peek("RETURN")||peek("NIL")||
                        peek("TRUE")||peek("FALSE")){
                    if(tokens.has(0)){
                        throw new ParseException("Expected Nonkeyword Identifier",
                                tokens.get(0).getIndex());
                    }
                    else throw new ParseException("Expected Nonkeyword Identifier",
                            tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                }
                ...
                if(peek("=")){
                    ...
                    if(peek("[")){
                        match("[");
                        if(peek("NIL") || peek("LET") || peek("SWITCH") || peek("IF") ||
                                peek("WHILE") || peek("RETURN") || peek("TRUE") || peek("FALSE") ||
                                peek(Token.Type.INTEGER) || peek(Token.Type.DECIMAL) || peek(Token.Type.CHARACTER)
                                || peek(Token.Type.STRING) || peek("(") || peek(Token.Type.IDENTIFIER)){
                            ...
                            if(peek("]")){
                                match("]");
                                Ast.Expression.PlcList seclist=new
                                        Ast.Expression.PlcList(firstlist);
                            ...
                            if(tokens.has(0)){
                                throw new ParseException("Expected Expresison",
                                        tokens.get(0).getIndex());
                            }
                            else throw new ParseException("Expected Expression",
                                    tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                        }
                    }else{
                        if(tokens.has(0)){
                        ...
                        else throw new ParseException("Expected [", tokens.get(-
                                1).getIndex()+tokens.get(-1).getLiteral().length());
                    }
                }
                else{
                ...
                if(tokens.has(0)){
                    throw new ParseException("Expected Identifier After Var",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Expected Identifier After Var",
                        tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
            }
        }
        else{
            ...
            else throw new ParseException("Expected LIST", tokens.get(-
                    1).getIndex()+tokens.get(-1).getLiteral().length());
        }
    }
    /**
    ...
    public Ast.Global parseMutable() throws ParseException {
        if(peek("VAR")){
            ...
            else throw new ParseException("Expected VAR", tokens.get(-
                    1).getIndex()+tokens.get(-1).getLiteral().length());
        }
        if(tokens.has(0)){
            throw new ParseException("Unknown VAR Error",
                    tokens.get(0).getIndex());
        }
        else throw new ParseException("Unknown VAR Error", tokens.get(-
                1).getIndex()+tokens.get(-1).getLiteral().length());
    }
    /**
    ...
    public Ast.Global parseImmutable() throws ParseException {
        if(peek("VAL")){
            ...
            if(peek(Token.Type.IDENTIFIER)){
                ...
                if(tokens.has(0)){
                    throw new ParseException("Expected Identifier After VAL",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Expected Identifier After VAL",
                        tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
            }
        }else{
            if(tokens.has(0)){
            ...
            else throw new ParseException("Expected VAL", tokens.get(-1).getIndex()
                    +tokens.get(-1).getLiteral().length());
        }
        if(tokens.has(0)){
            throw new ParseException("Expected VAR", tokens.get(0).getIndex());
        }
        else throw new ParseException("Expected VAR", tokens.get(-1).getIndex()
                +tokens.get(-1).getLiteral().length());
    }
    /**
        ...
        if(peek("FUN")) {
            ...
            if (peek(Token.Type.IDENTIFIER)) {
                if(peek("VAL")||peek("VAR")||peek("LIST")){
                    if(tokens.has(0)){
                        throw new ParseException("Global Keyword In Function Name",
                                tokens.get(0).getIndex());
                    }
                ...
                if(peek("FUN")){
                    if(tokens.has(0)){
                        throw new ParseException("Unexpected FUN Keyword",
                                tokens.get(0).getIndex());
                    }
                    else throw new ParseException("Unexpected FUN Keyword",
                            tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                }
                if(peek("LET")||peek("SWITCH")||peek("CASE")||peek("DEFAULT")||
                        peek("END")||peek("IF")||peek("DO")||peek("ELSE")||peek("WHILE")||peek("RETURN")||
                        peek("NIL")||peek("TRUE")||peek("FALSE")){
                    if(tokens.has(0)){
                        throw new ParseException("Expected Nonkeyword Identifier",
                                tokens.get(0).getIndex());
                    }
                ...
                if (peek("(")) {
                    ...
                    while (!peek(")") && peek(Token.Type.IDENTIFIER)) {
                        if (peek(Token.Type.IDENTIFIER)) {
                            if(peek("VAL")||peek("VAR")||peek("LIST")){
                                ...
                                else throw new ParseException("Global Keyword In Function Parameter", tokens.get(-1).getIndex()+tokens.get(-
                                        1).getLiteral().length());
                            }
                            if(peek("FUN")){
                                if(tokens.has(0)){
                            ...
                            if(peek("LET")||peek("SWITCH")||peek("CASE")||
                                    peek("DEFAULT")||peek("END")||peek("IF")||peek("DO")||peek("ELSE")||peek("WHILE")||
                                    peek("RETURN")||peek("NIL")||peek("TRUE")||peek("FALSE")){
                                if(tokens.has(0)){
                            ...
                            else throw new ParseException("Expected Identifier",
                                    tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                        }
                    }
                    if (peek(")")) {
                        ...
                        if (peek("DO")) {
                            match("DO");
                            if ((peek("NIL") || peek("LET") || peek("SWITCH") ||
                                    peek("IF") || peek("WHILE") || peek("RETURN") || peek("TRUE") || peek("FALSE") ||
                                    peek(Token.Type.INTEGER) || peek(Token.Type.DECIMAL) || peek(Token.Type.CHARACTER)
                                    || peek(Token.Type.STRING) || peek("(") || peek(Token.Type.IDENTIFIER))) {
                                if(peek("VAL")||peek("VAR")||peek("LIST")){
                                    if(tokens.has(0)){
                                    ...
                                    else throw new ParseException("Global Keyword In Function Block", tokens.get(-1).getIndex()+tokens.get(-
                                            1).getLiteral().length());
                                }
                                if(peek("FUN")){
                                    if(tokens.has(0)){
                                ...
                                if (peek("END")) {
                                    ...
                                    else throw new ParseException("Expected END",
                                            tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                                }
                            } else {
                                if(tokens.has(0)){
                                    throw new ParseException("Expected Block",
                                            tokens.get(0).getIndex());
                                }
                                else throw new ParseException("Expected Block",
                                        tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                            }
                        } else {
                            if(tokens.has(0)){
                            ...
                            else throw new ParseException("Expected DO",
                                    tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
                        }
                    } else {
                        if(tokens.has(0)){
                    ...
                    else throw new ParseException("Expected (FUN)", tokens.get(-
                            1).getIndex()+tokens.get(-1).getLiteral().length());
                }
            } else {
                if(tokens.has(0)){
                ...
                else throw new ParseException("Expected Identifier After FUN",
                        tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
            }
        }
        else{
    ...
    public List<Ast.Statement> parseBlock() throws ParseException {
        ...
        while ((!peek("END") && !peek("ELSE") && !peek("CASE") && !peek("DEFAULT")
                && !peek("DO") && !peek("LIST") && !peek("VAR") && !peek("VAL")) && ((peek("NIL")
                || peek("LET") || peek("SWITCH") || peek("IF") || peek("WHILE") || peek("RETURN")
            ...
            if(peek("LIST")){
                if(tokens.has(0)){
                    throw new ParseException("Unexpected LIST",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Unexpected LIST", tokens.get(-
                        1).getIndex()+tokens.get(-1).getLiteral().length());
            }
            if(peek("VAR")){
                if(tokens.has(0)){
                    throw new ParseException("Unexpected VAR",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Unexpected VAR", tokens.get(-
                        1).getIndex()+tokens.get(-1).getLiteral().length());
            }
            if(peek("VAL")) {
                if(tokens.has(0)){
                    throw new ParseException("Unexpected VAL",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Unexpected VAL", tokens.get(-
                        1).getIndex()+tokens.get(-1).getLiteral().length());
            }
            if(peek("FUN")){
                if(tokens.has(0)){
...
//EXTRA EXCEPTION CHECKING NEEDED,
    ...
    public Ast.Statement parseStatement() throws ParseException {
        if (peek("LET")) {
            ...
            if(peek("VAL")||peek("VAR")||peek("LIST")){
                if(tokens.has(0)){
                    throw new ParseException("Global Keyword In Statement",
                            tokens.get(0).getIndex());
                }
                else throw new ParseException("Global Keyword In Statement",
                        tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
            }
            if(peek("FUN")){
                if(tokens.has(0)){
        ...
        throw new ParseException("Really should not hit here",
                tokens.get(0).getIndex());
    }
    /**
    ...
    public Ast.Statement.Declaration parseDeclarationStatement() throws
        ...
        match("LET");
        if(peek(Token.Type.IDENTIFIER)){
            if(peek("LIST")||peek("VAR")||peek("VAL")||peek("FUN")||peek("LET")||
                    peek("SWITCH")||peek("CASE")||peek("DEFAULT")||peek("END")||peek("IF")||
                    peek("DO")||peek("ELSE")||peek("WHILE")||peek("RETURN")||peek("NIL")||
                    peek("TRUE")||peek("FALSE")){
                if(tokens.has(0)){
                    throw new ParseException("Expected Nonkeyword Identifier",
                            tokens.get(0).getIndex());
    ...
    public Ast.Statement.If parseIfStatement() throws ParseException {
        if(peek("IF")) {
            ...
            if (!peek("NIL") && !peek("TRUE") && !peek("FALSE") && !
                    ...
                    peek(Token.Type.CHARACTER) && !peek(Token.Type.STRING) && !peek("(") && !
    ...
    public Ast.Statement.Switch parseSwitchStatement() throws ParseException {
        ...
        if(!peek("NIL")&&!peek("TRUE")&&!peek("FALSE")&&!
                ...
                peek(Token.Type.STRING)&&!peek("(")&&!peek(Token.Type.IDENTIFIER)){
            if(tokens.has(0)){
                throw new ParseException("Expected Expression After SWITCH",
                        tokens.get(0).getIndex());
            }
            else throw new ParseException("Expected Expression After SWITCH",
                    tokens.get(-1).getIndex()+tokens.get(-1).getLiteral().length());
        }
        Ast.Expression expr=parseExpression();
        ...
        if(peek("DEFAULT")){
            ...
            if(tokens.has(0)){
                throw new ParseException("Expected DEFAULT Case",
                        tokens.get(0).getIndex());
            }
    ...
    public Ast.Expression parseExpression() throws ParseException {
        if(peek("LIST")||peek("VAR")||peek("VAL")||peek("FUN")||peek("LET")||
                peek("SWITCH")||peek("CASE")||peek("DEFAULT")||peek("END")||peek("IF")||
                peek("DO")||peek("ELSE")||peek("WHILE")||peek("RETURN")){
            if(tokens.has(0)){
                throw new ParseException("Unexpected Keyword",
                        tokens.get(0).getIndex());
            }
            else throw new ParseException("Unexpected Keyword", tokens.get(-
                    1).getIndex()+tokens.get(-1).getLiteral().length());
        }
...
private char processCharLiteral(String literal) {
    ...
    throw new IllegalArgumentException("Invalid character literal: " +
            literal);
}
